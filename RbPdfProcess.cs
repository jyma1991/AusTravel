...
namespace AusTravel
{
    ...
    {       
        ...
        private void WriteRegularInfo(Worksheet workSheet)
        {
            ...
            TitleStyle(workSheet, "G10", "G10", "Agent Ref:", true,9);
            TitleStyle(workSheet, "G12", "G12", "Require:", true,10);
            TitleStyle(workSheet, "G14", "G14", "Vehicle:", true,10);

            TitleStyle(workSheet, "H10", "H10", dicBookInfo["Booking Ref"], true, 9);
            ...
            TitleStyle(workSheet, "A15", "C15", "Itinerary", true,14);

            TitleStyle(workSheet, "D15", "E15", "Quote Items", true, 14);

            TitleStyle(workSheet, "F15", "F15", "U-Price", true, 14);
            TitleStyle(workSheet, "G15", "G15", "Qty", true, 14);
            TitleStyle(workSheet, "H15", "H15", "Discount", true, 14);
            TitleStyle(workSheet, "I15", "I15", "T-Price", true, 14);
        ...
        private void GetQuoteAndPrice(SortedList<DateTime, string> ICainsTravel, int iLine)
        {
            ...
            {
                if (ICainsTravel.Values[i].Contains("Palm Cove"))
                {
                    NoteStyle(workSheet, "D" + (iLine), "E" + (iLine), " Palm Cove Addition", false, 10);
                    NoteStyle(workSheet, "F" + (iLine), "F" + (iLine), dicprice["northern beach surcharge  "].ToString(), false, 10);
                    iLine++;
                }
                if (ICainsTravel.Values[i].Contains("Arrive Cairns") && firstAppear("Arrive Cairns"))
                {
                    dt = GetHours(ICainsTravel.Keys[i], ICainsTravel.Keys[ICainsTravel.Keys.Count - 1]);
        ...
        private int GetHours(DateTime timeStart, DateTime timeEnd)
        {
            if ((timeEnd - timeStart).Minutes >= 10)
            {
              return (timeEnd - timeStart).Hours + 1;
            }
            else
            {
                return (timeEnd - timeStart).Hours;
            }

            
        }
        String isappear = null;
        private Boolean firstAppear(String aa) 
        {   
            if(isappear != aa)
            {
                isappear =aa;
      ...
      //      Worksheet workSheet = (Worksheet)travel.Globals.ThisWorkbook.ActiveSheet;
        ...
        private void dropDown_SelectionChanged(object sender, RibbonControlEventArgs e)
        {
